...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        """
        counts = np.array([self.hist[self.preprocessor(s)] for s in states])
        return counts
    ...
    def get_prob(self, states):
        ...
        """
        probs = np.array([self.hist[self.preprocessor(s)] for s in states]) / self.total
        return probs
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.kl = tf.add(*[tf.distributions.kl_divergence(post, self.prior) for post in [self.encoder1, self.encoder2]])
        assert len(self.log_likelihood.shape) == len(self.likelihood.shape) == len(self.kl.shape) == 1

        # raise NotImplementedError
        self.elbo = tf.reduce_mean(self.log_likelihood - self.kl_weight * self.kl, axis=-1)
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        with tf.variable_scope(scope):
            z_logstd = tf.Variable(np.zeros(int(z_size), dtype=np.float32))
    ...
    def make_prior(self, z_size):
        ...
        return tfp.distributions.MultivariateNormalDiag(loc=prior_mean, scale_diag=np.exp(prior_logstd))
